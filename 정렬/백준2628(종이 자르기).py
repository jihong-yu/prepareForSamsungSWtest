width, height = map(int, input().split()) #가로 세로 길이 입력
N = int(input()) #점 개수 입력
info_width = [0, width]  # 가로 길이를 구하기위해 세로점들을 저장할 리스트
info_height = [0, height]  # 세로 길이를 구하기 위해 가로점들을 저장할 리스트
for i in range(N):  # 입력 받은 개수 만큼 반복문 돈다.
    ...
    if temp[0] == 0:  # 만약 가로점을 입력받았다면
        ...
        info_width.append(temp[1])  # 가로 길이 리스트에 값들을 저장해준다.
